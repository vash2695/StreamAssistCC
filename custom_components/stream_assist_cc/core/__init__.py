    ...
    async def internal_event_callback(event: PipelineEvent):
        ...
        if event.type == PipelineEventType.STT_END:
            ...
            if re.match(r'^(' + '|'.join(CANCELLATION_PHRASES) + r')$', stt_text.strip()):
                ...
                _LOGGER.debug("Ending pipeline run due to cancellation phrase")
                await pipeline_run.end()
            ...
            if player_entity_id:
                ...
                # End the pipeline run after TTS playback
                _LOGGER.debug("Ending pipeline run after TTS playback")
                await pipeline_run.end()
